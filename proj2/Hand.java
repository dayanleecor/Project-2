...
public int compareTo( Object otherHandObject ) {
 ...
 int p1 = this.evaluateHand(); 
	int p2 = otherHand.evaluateHand();
	
	int result = p1 - p2;
	if (result < 0) {
		System.out.println("Points: " + p1 + " to " + p2);
		System.out.println("Player 1 Wins!");
	}
	else if(result > 0) {
		System.out.println("Points: " + p2 + " to " + p1);
 ...
 */
 ...
 public Vector<Card> asVector() {return this.hand;}
 ...
 */
 public List getCurrentHand() {
	...
	/**
	* Generates and returns either 1 or 2 to choose between drawing 
	* from deck or stack.
	*/
	public int playDraw() {
		int min = 1, max = 2, rnum;
	...
	/**
	* Generates and returns either 3 or 4 to choose between discarding 
	* by laying a set or on stack.
	*/
	public int playDiscard() {
		int min = 3, max = 4, rnum;
	...
	/**
	* Generates and returns an index to pick a card from hand to
	* be discarded.
	*/
	public int makeIndex() {
		int min = 1, max = this.getNumberOfCards() - 1, rnum;
